...
function BorderSection({ title, value, color, onChange, defaultOpen = true, description, open, onOpenChange }: { title: string; value: BorderOptions | undefined; color: string; onChange: (value: BorderOptions) => void; defaultOpen?: boolean; description?: string; open?: boolean; onOpenChange?: (open: boolean) => void }) {
  const current = { visible: true, color, width: 1, radius: 100, ...value };
  return <AccordionItem title={title} description={description} defaultOpen={defaultOpen} open={open} onOpenChange={onOpenChange}><div className="fields-row"><SwitchField label="是否显示" value={current.visible} onChange={visible => onChange({ ...current, visible })} /><ColorField label="颜色" value={current.color} onChange={borderColor => onChange({ ...current, color: borderColor })} /><NumberField label="线宽" value={current.width} onChange={width => onChange({ ...current, width })} /><NumberField label="半径" value={current.radius} hint="距离中心点的距离" onChange={radius => onChange({ ...current, radius })} /></div></AccordionItem>;
}

function TextSection({ title, value, color, mode, onChange, defaultOpen = true, description, open, onOpenChange }: { title: string; value: TextOptions | undefined; color: string; mode: 'surround' | 'horizontal'; onChange: (value: TextOptions) => void; defaultOpen?: boolean; description?: string; open?: boolean; onOpenChange?: (open: boolean) => void }) {
  const current = { visible: true, color, fontSize: 24, fontWeight: 600, text: '', distance: 0, radius: 120, startDegree: 25, ...value };
  return <AccordionItem title={title} description={description} defaultOpen={defaultOpen} open={open} onOpenChange={onOpenChange}><div className="fields-row"><SwitchField label="是否显示" value={current.visible} onChange={visible => onChange({ ...current, visible })} /><ColorField label="颜色" value={current.color} onChange={textColor => onChange({ ...current, color: textColor })} /><TextField label="文案" value={current.text} onChange={text => onChange({ ...current, text })} /><NumberField label="字体大小" value={current.fontSize} onChange={fontSize => onChange({ ...current, fontSize })} /><NumberField label="字体粗细" value={Number(current.fontWeight)} onChange={fontWeight => onChange({ ...current, fontWeight })} />{mode === 'surround' ? <><NumberField label="半径" value={current.radius} onChange={radius => onChange({ ...current, radius })} /><NumberField label="字体开始角度" value={current.startDegree} onChange={startDegree => onChange({ ...current, startDegree })} /></> : <NumberField label="距离" value={current.distance} hint="距离中心点的距离" onChange={distance => onChange({ ...current, distance })} />}</div></AccordionItem>;
}
...
export function SealForm({ options, onChange }: SealFormProps) {
  ...
  return <div className="form-panel">
    <AccordionItem title="基本配置" description="类型、形状与画布" {...section('basics')}>
    ...
    <BorderSection title="边线配置" value={options.border} color={color} onChange={border => patch({ border })} {...section('border')} description="外圈边框样式" />
    <BorderSection title="内边线配置" value={options.innerBorder} color={color} onChange={innerBorder => patch({ innerBorder })} {...section('inner-border')} description="内圈边框样式" />
    <BorderSection title="内环线配置" value={options.innerLoopLine} color={color} onChange={innerLoopLine => patch({ innerLoopLine })} {...section('loop')} description="辅助环线样式" />

    <AccordionItem title="五角星配置" description="中心图形样式" {...section('star')}>
    ...
    <TextSection title="主文字配置" value={options.text} color={color} mode="surround" onChange={value => patchText('text', value)} {...section('text')} description="环绕印章上方的主文案" />
    <TextSection title="副文字配置" value={options.subText} color={color} mode="horizontal" onChange={value => patchText('subText', value)} {...section('sub-text')} description="印章中心上方的副文案" />
    <TextSection title="中心文字配置" value={options.centerText} color={color} mode="horizontal" onChange={value => patchText('centerText', value)} {...section('center-text')} description="印章中心的文字" />
    <TextSection title="序列号配置" value={options.serNo} color={color} mode="surround" onChange={value => patchText('serNo', value)} {...section('serial')} description="底部环绕序列号" />
  </div>;
}
